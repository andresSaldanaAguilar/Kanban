                ...
                if (mysqli_num_rows($consulRes) != 1)
                    ...
                    if (mysqli_num_rows($consulRes) != 1)
                        ...
                        if($idNewUser == $inUser['idUsuario'])
                            echo "<script type='text/javascript'>alert('No puedes eliminarte a ti mismo'); document.location='MenuTablero.php'</script>";
                            ...
                            if (mysqli_num_rows($consulRes) != 1)
                                echo "<script type='text/javascript'>alert('Usuario a eliminar no existente'); document.location='MenuTablero.php'</script>";
                            else{
                                $consulQuery = "select * from miembrouserport where idUsuario='" . $idNewUser . "' and idPortafolio='" . $inPortafolio['idPortafolio'] . "';";
                                $consulRes = mysqli_query($conexion, $consulQuery) or die ("Algo ha ido mal".mysql_error());
            
                                if (mysqli_num_rows($consulRes) != 1)
                                    echo "<script type='text/javascript'>alert('Usuario a eliminar no encontrado'); document.location='MenuTablero.php'</script>";
                                else{
                                    $execQuery = "delete from miembrouserport where idUsuario='" . $idNewUser . "' and idPortafolio='" . $inPortafolio['idPortafolio'] . "';";
                                    $execRes = mysqli_query($conexion, $execQuery) or die ("Algo ha ido mal".mysql_error());
                                    
                                    echo "<script type='text/javascript'>alert('Usuario eliminado exitosamente'); document.location='MenuTablero.php';</script>";
                                }
                            }
                        }
                        else
